...
class CompositeTest extends \PHPUnit_Framework_TestCase
{
    public function testMergeNestedGroupsIfNoExplicitParentGroup()
    {
        $constraint = new ConcreteComposite(array(
            new NotNull(array('groups' => 'Robot')),
            new NotBlank(array('groups' => array('Robot', 'Strict'))),
        ));

        $this->assertEquals(array('Robot', 'Strict'), $constraint->groups);
        $this->assertEquals(array('Robot'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Robot', 'Strict'), $constraint->constraints[1]->groups);
    }
    ...
    public function testSetImplicitNestedGroupsIfExplicitParentGroup()
    {
        $constraint = new ConcreteComposite(array(
            ...
            'groups' => array('Robot', 'Strict'),
        ));

        $this->assertEquals(array('Robot', 'Strict'), $constraint->groups);
        $this->assertEquals(array('Robot', 'Strict'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Robot', 'Strict'), $constraint->constraints[1]->groups);
    }
    ...
    public function testFailIfExplicitNestedGroupsNotSubsetOfExplicitParentGroups()
    {
        new ConcreteComposite(array(
            'constraints' => array(
                new NotNull(array('groups' => array('Robot', 'Foobar'))),
            ),
            'groups' => array('Robot', 'Strict'),
        ));
    }
    ...
    public function testImplicitGroupNamesAreForwarded()
    {
        ...
        $this->assertEquals(array('Robot', 'Strict', 'ImplicitGroup'), $constraint->groups);
        $this->assertEquals(array('Robot', 'ImplicitGroup'), $constraint->constraints[0]->groups);
        $this->assertEquals(array('Strict'), $constraint->constraints[1]->groups);
    }
